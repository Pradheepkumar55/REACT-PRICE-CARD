...
function createFeatures( object ) { 
        const lists = object.cardData.features.map( ( feature, ind ) => (
            <p key={ind} className={( feature.isIncluded ) ? "" : "notIncluded"}>
                <span className='span'>{( feature.isIncluded ) ? <i className='bx bx-check'></i> : <i className='bx bx-x'></i>}</span>{feature.text}</p>
        ) )
        return lists;
    }
        ...
        <div>
            ...
            <div className='container'>
                <div className='Header'>
                    <h5 style={{ color: "gray",marginBottom:"3px" }}>{props.cardData.tittle}</h5>
                ...
                <div className='footer'>
                    ...
                    <div><button type='button' className={btnHighLight( props)}>BUTTON</button></div>
                </div>  
            </div>
          </div>
        </>
